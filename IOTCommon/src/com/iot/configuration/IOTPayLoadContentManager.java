 ...
 * 
 */
public class IOTPayLoadContentManager {

	/*
	 * Resource bundle to hold configuration key values
	 */

	private static ResourceBundle payloadContentFile;

	private static Map<String, String> payloadContents;

	static {
		try {

			payloadContentFile = ResourceBundle.getBundle("com.iot.iotservice.configuration.properties.payloadcontent");
	...
	public static String getParameterValue(String key){
		try {		
			String value = (String)payloadContentFile.getObject(key);
			return value;
	...
	public static Map<String, String> getPayLoadContent() {
		if (null == payloadContents) {
			payloadContents = readResourceBundleintoMap(payloadContentFile);
		}
